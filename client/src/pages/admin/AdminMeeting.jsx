...
import { FiSend, FiUsers, FiMessageSquare, FiSearch, FiCircle, FiHash, FiMoreVertical, FiPaperclip, FiSmile, FiTrash2, FiFile, FiDownload, FiX, FiEdit2, FiArrowLeft, FiImage, FiBell, FiInfo, FiChevronRight } from 'react-icons/fi';
...
export default function AdminMeeting() {
  ...
  const fetchMessages = async () => {
    ...
    try {
      let query = supabase
        .from('admin_messages')
        .select('*, sender:admins!sender_id(username, district_id, districts(name))');
      ...
      .on('postgres_changes', { 
        ...
        if (isGroupMsg || isRelevantPrivate) {
          ...
          const { data: senderData } = await supabase
            .from('admins')
            .select('username, district_id, districts(name)')
          ...
          setMessages(prev => [...prev, { 
            ...
            sender: {
              ...
              district: senderData?.districts?.name,
              photo_url: null
            } 
          }]);
          ...
          if (newMessage.receiver_id === user.id && (!selectedAdmin || selectedAdmin.id !== newMessage.sender_id)) {
            ...
            if (!isMuted) showToast(`New message from ${senderData?.username}`, 'info');
          }
        }
      })
      .on('postgres_changes', {
  ...
  return (
    <div className="min-h-screen mountain-bg pt-16 md:pt-20 pb-0 md:pb-6 px-0 md:px-4">
      ...
      <div className="max-w-7xl mx-auto h-[calc(100vh-80px)] md:h-[calc(100vh-120px)] flex gap-6 animate-fade-in relative overflow-hidden">
        
        {/* Sidebar */}
        <div className={`${showMobileChat ? 'hidden' : 'flex'} md:flex w-full md:w-80 bg-white/80 backdrop-blur-xl md:rounded-[2.5rem] shadow-xl border-x md:border border-white/50 flex-col overflow-hidden`}>
          <div className="p-6 border-b border-gray-100">
            ...
            <div className="relative mt-4">
              <input 
                ...
                className="w-full bg-gray-50 border-none rounded-2xl py-3 pl-10 pr-4 text-xs font-bold focus:ring-2 focus:ring-emerald-500/20 focus:bg-white transition-all shadow-inner"
              />
          ...
          >
            {loading ? (
              ...
              filteredMessages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} animate-slide-in-up group`}>
          ...
          <div className="p-2 md:p-6 bg-gray-50/80 backdrop-blur-md border-t border-gray-100">
            <form onSubmit={sendMessage} className="flex items-center gap-1.5 md:gap-4 bg-white p-1.5 pl-3 md:pl-6 rounded-[1.5rem] md:rounded-full shadow-lg shadow-gray-200/50 border border-transparent focus-within:border-emerald-200 focus-within:ring-4 focus-within:ring-emerald-500/10 transition-all">
              ...
              <input 
                ...
                disabled={uploading}
                className="flex-1 bg-transparent border-none focus:ring-0 text-xs md:text-sm font-bold text-gray-800 placeholder:text-gray-400 py-2 md:py-3"
              />
